...
mortality_rate_ratio_fig <- excess_deaths_year %>% ggplot(aes(x=Year, y=ratio_adj_excess_deaths_rate, color=Gender)) + geom_line(size=1.25) + ylab("Mortality Rate Ratio (Black / White)") + scale_y_continuous(limits = c(0.85, max(excess_deaths_year$ratio_adj_excess_deaths_rate + 0.5, na.rm=TRUE)), breaks=seq(0.75, max(excess_deaths_year$ratio_adj_excess_deaths_rate + 0.5, na.rm=TRUE), 0.25)) + geom_hline(yintercept=1, linetype="dashed") + sizing_theme + scale_color_manual(values=c("maroon", "navy")) + ggtitle("Black-White Age-Adjusted Mortality Rate Ratio") + year_label + panel_theme
...
age_adj_deaths_race_sex_fig <-  edy %>% ggplot(aes(x=Year, y=value, color=Race, shape=Gender, linetype=Gender)) + geom_line(size=0.5) + geom_point(size=2) + panel_theme + sizing_theme + year_label + scale_y_continuous(limits=c(0, max(edy$value, na.rm=TRUE)*1.15), breaks=seq(0, max(edy$value, na.rm=TRUE)*1.15, 10)) + ylab("Mortality Rate per 100K Individuals") + ggtitle("Estimated Age-Adjusted Mortality Rate") + scale_color_manual(values=cbb[1:2])
...
cumulative_excess_deaths_fig <- cumedb %>% ggplot(aes(x=Year, y=edb, color=Gender)) + geom_point() + geom_line() + year_label + panel_theme + sizing_theme + ggtitle("Estimated Cumulative Number of Excess Deaths Among the Black Population") + ylab("Excess Number of Deaths (Thousands)") + scale_y_continuous(limits=c(0,max(cumedb$edb, na.rm=TRUE)*1.15), breaks=seq(0,max(cumedb$edb, na.rm=TRUE)*1.15,25)) + scale_color_manual(name="", values=c("maroon", "navy"))
...
cumulative_excess_net_fig <- cum_excess %>% ggplot(aes(x=Year, y=edb)) + geom_point() + geom_line() + year_label + panel_theme + sizing_theme + ggtitle("Estimated Cumulative Number of Excess Deaths Among the Black Population\nMales and Females Combined") + ylab("Excess Number of Deaths (Thousands)") + scale_y_continuous(limits=c(0,max(cum_excess$edb, na.rm=TRUE)*1.15), breaks=seq(0,max(cum_excess$edb, na.rm=TRUE)*1.15,25)) + scale_color_manual(name="", values=c("black"))
