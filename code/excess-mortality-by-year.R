...
#input file
inputfile <- file.path("../data", "export_age_adjusted_deaths_race_gender_year_se.txt")
inputfile2 <- file.path("../data", "export_deaths_race_gender_age_year.txt")
...
#plot output dir
plotdir <- "../plots/excess-mortality-by-year"
...
mortality_rate_ratio_fig <- excess_deaths_year %>% ggplot(aes(x=Year, y=ratio_adj_excess_deaths_rate, color=Gender)) + geom_line(size=1.25) + ylab("Mortality Rate Ratio (Black / White)") + scale_y_continuous(limits = c(0.85, 1.5), breaks=seq(0.85, 1.5, 0.05)) + geom_hline(yintercept=1, linetype="dashed") + sizing_theme + scale_color_manual(values=c("maroon", "navy")) + ggtitle("Black-White Age-Adjusted Mortality Rate Ratio") + year_label + panel_theme
