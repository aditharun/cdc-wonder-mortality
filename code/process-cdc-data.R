...
filtered_data <- filtered_data %>% group_by(Race, `Hispanic Origin`, `Five-Year Age Groups`, `Cause of death Code`, Gender, Year) %>% summarize(Deaths = sum(Deaths)) %>% ungroup()
...
dat <- dat %>% group_by(Race, Gender, Year, `Hispanic Origin`, `Cause of death Code`) %>% summarize(AA_rate = sum(AA_rate), Deaths = sum(Deaths), Population = sum(Population)) %>% mutate(`Crude Rate`= (Deaths / Population) * 100000) %>% mutate(Gender = ifelse(Gender == "Female", "F", "M")) %>% ungroup()
